...
class DiseaseCJ:
    ...
    def __init__(self, input_dir="TEST_PHOTO", output_dir="CJ_debug_output", pipe_diameter_pixels=None):
        ...
        Args:
            ...
            output_dir (str): Directory to save debug images and results. Defaults to 'CJ_debug_output'.
    ...
    def detect_sediment(self, img_path):
        """Detect sediment in the image, save result image and text report.
        ...
        self._save_debug_image(sediment_mask, f"{img_name}_04_eroded_sediment_mask.jpg", 'gray')
        ...
        self._save_debug_image(sediment_inside_circle, f"{img_name}_06_sediment_inside_circle.jpg", 'gray')

        # Calculate areas
        half_circle_area = cv2.countNonZero(circle_mask_lower)
        sediment_area = cv2.countNonZero(sediment_inside_circle)
        print(
            f"✅ Circle area: {half_circle_area * 2} px, Lower half circle area: {half_circle_area} px, Sediment area: {sediment_area} px")
        ...
        result = self._generate_result(sediment_ratio)
        ...
        info = [
            f"Sediment Ratio: {result['Sediment Area Ratio (%)']:.1f}%",
        ...
        result['debug_images'] = [
            ...
            f"{img_name}_02_blurred_hough.jpg" if os.path.exists(
                os.path.join(self.output_dir, f"{img_name}_02_blurred_hough.jpg")) else None,
            f"{img_name}_03_hough_circle.jpg" if os.path.exists(
                os.path.join(self.output_dir, f"{img_name}_03_hough_circle.jpg")) else None,
            f"{img_name}_02_canny_projection.jpg" if os.path.exists(
                os.path.join(self.output_dir, f"{img_name}_02_canny_projection.jpg")) else None,
            f"{img_name}_03_projection_lines.jpg" if os.path.exists(
                os.path.join(self.output_dir, f"{img_name}_03_projection_lines.jpg")) else None,
            f"{img_name}_04_eroded_sediment_mask.jpg",
            f"{img_name}_05_circle_mask_lower.jpg",
            f"{img_name}_06_sediment_inside_circle.jpg",
            f"{img_name}_result.jpg"
        ]
        result['debug_images'] = [img for img in result['debug_images'] if
                                  img and os.path.exists(os.path.join(self.output_dir, img))]
    ...
    def process_directory(self):
        ...
        for img_path in image_files:
            try:
                ...
                result, result_img_path, result_txt_path = self.detect_sediment(img_path)
...
if __name__ == "__main__":
    detector = DiseaseCJ(input_dir="TEST_PHOTO", output_dir="CJ_debug_output")
